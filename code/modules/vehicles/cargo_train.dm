...
/obj/vehicle/train/cargo/engine/examine(mob/user)
	...
	to_chat(user, "The charge meter reads [cell? round(percent2(cell), 0.01) : 0]%")
...
/obj/vehicle/train/cargo/trolley/load(var/atom/movable/C)
	if(ismob(C) && !passenger_allowed)
		return 0
	if(!ismachine(C) && !istype(C,/obj/structure/closet) && !istype(C,/obj/structure/largecrate) && !istype(C,/obj/structure/reagent_dispensers) && !istype(C,/obj/structure/ore_box) && !ishuman(C))
		return 0
