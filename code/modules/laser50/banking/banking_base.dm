...
/*-----Basic variables for the database & money_account-----*/
var/database/BankofNT = new("data/BankofNT.db")
...
//Temporary datum to store money information.
/datum/money_account
	var/owner_name = ""
...
/proc/attempt_account_access(var/attempt_account_number, var/attempt_pin_number, var/security_level_passed = 0)
	var/datum/money_account/D = get_account(attempt_account_number)
	if(D?.security_level <= security_level_passed && (!D.security_level || D.account_pin == attempt_pin_number) )
		return D
