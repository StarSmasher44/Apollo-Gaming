...
#define HUMAN_MAX_OXYLOSS 4 //Defines how much oxyloss humans can get per tick. A tile with no air at all (such as space) applies this value, otherwise it's a percentage of it.
		...
		if (radiation > 50)
			...
			if(!isSynthetic())
				if(prob(5) && prob(100 * RADIATION_SPEED_COEFFICIENT))
					...
					if(!lying)
						emote("collapse")
				if(prob(5) && prob(100 * RADIATION_SPEED_COEFFICIENT) && GET_BODY_TYPE(src) == SPECIES_HUMAN) //apes go bald
...
/mob/living/carbon/human/handle_environment(datum/gas_mixture/environment)
	...
	if(isspace(src.loc)) //being in a closet will interfere with radiation, may not make sense but we don't model radiation for atoms in general so it will have to do for now.
...
/mob/living/carbon/human/proc/handle_shock()
	...
	if(traumatic_shock >= max(30, 0.8*shock_stage))
		...
		shock_stage = min(shock_stage, 170)
		var/recovery = 1
