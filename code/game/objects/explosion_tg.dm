...
/datum/explosion/New(atom/epicenter, devastation_range, heavy_impact_range, light_impact_range, flash_range, adminlog, ignorecap, flame_range, silent, smoke)
	...
	if(adminlog)
		message_admins("Explosion with size ([devastation_range], [heavy_impact_range], [light_impact_range], [flame_range]) in area: [epi_area] (<A HREF='?_src_=holder;adminplayerobservecoodjump=1;X=[epicenter.x];Y=[epicenter.y];Z=[epicenter.z]'>JMP</a>)")
	...
	for(var/TI in affected_turfs)
		...
		if(flame_dist && prob(40) && !isspaceturf(T) && !T.density)
			new /obj/effect/fire(T) //Mostly for ambience!
...
/client/proc/check_bomb_impacts()
	...
	sleep(100)
	wipe_color_and_text(wipe_colours)
