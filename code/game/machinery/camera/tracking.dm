...
/mob/living/silicon/robot/tracking_status()
	. = ..()
	if(. == TRACKING_NO_COVERAGE)
		return camera?.can_use() ? TRACKING_POSSIBLE : TRACKING_NO_COVERAGE
...
/mob/living/carbon/human/tracking_status()
	...
	if(. == TRACKING_NO_COVERAGE)
		...
		if((T?.z in GLOB.using_map.station_levels) && hassensorlevel(src, SUIT_SENSOR_TRACKING))
